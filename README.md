...
- 윤현진
  ...
  그동안은 혼자 간단한 실습만 하다가 처음으로 협업을 하게 되니 어려움이 많았습니다. 특히 초반에는 API 명세서가 정확히 어떤 건지 잘 몰라서 더 막막했던 것 같습니다. 하지만 프로젝트를 하나씩 구현하면서 진짜 공부는 실전이다라는 걸 느꼈습니다.
  이론만 배우고 직접 적용하지 않으면 결국 안 배운 것과 다를 게 없다는 생각도 들었습니다.

  이번 프로젝트에서 인상깊었던 것은 시간이 좀 걸렸지만 무한 스크롤을 직접 구현해본 것, 챗봇 기능을 만들어본 것, 그리고 AWS S3와 배포를 직접 해본 경험이었습니다.
  배포할 때는 환경 변수 설정 오류나 인스턴스 연결이 끊기는 문제 같은 다양한 이슈가 생겼습니다. 그걸 해결하려고 인스턴스를 재부팅하고, 탄력적 IP도 할당해보면서 AWS를 조금 더 이해할 수 있게 되었습니다.

  프로젝트 기간은 짧았지만, 팀원들과 함께 문제를 논의하고 해결책을 찾아가는 과정에서 많은 걸 배울 수 있었습니다.
